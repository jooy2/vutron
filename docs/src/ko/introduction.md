...
- ⚡️ 다국어 지원
- ⚡️ 테마 지원 (다크 & 라이트)
- ⚡️ Vuetify 기반의 기본적인 레이아웃 관리
- ⚡️ Pinia 스토어를 활용한 글로벌 상태 관리
- ⚡️ 트레이 아이콘 지원 (플로팅 트레이 창 옵션 포함)
- ⚡️ 창 관리자를 통한 멀티 윈도우 지원 (상수 하나로 켜고 끄기)
- ⚡️ 기본으로 하나의 인스턴스만 실행, 두 번째 실행은 이미 열린 창을 앞으로
- ⚡️ 끝단까지 타입이 적용된 IPC 채널, 호출과 핸들러가 어긋나면 빌드 실패
- ⚡️ IPC를 통한 네이티브 파일 다이얼로그 및 외부 링크 예제
- ⚡️ 기본 적용된 보안 설정: 컨텍스트 격리, 채널 화이트리스트, CSP, 네비게이션 가드
